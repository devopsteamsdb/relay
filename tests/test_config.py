...
@contextmanager
def announce(test_name: str):
    """Print a friendly status line for each test."""
...
class ConfigVersionTests(unittest.TestCase):
    def test_apply_latest_version_replaces_placeholder(self):
        with announce("apply_latest_version_replaces_placeholder"):
            tool = {
                "name": "Docker",
    ...
    def test_apply_latest_version_without_source_keeps_placeholder(self):
        with announce("apply_latest_version_without_source_keeps_placeholder"):
    ...
    def test_fetch_latest_version_github_release_parses_tag(self):
        with announce("fetch_latest_version_github_release_parses_tag"):
    ...
    def test_fetch_latest_version_hashicorp_checkpoint(self):
        with announce("fetch_latest_version_hashicorp_checkpoint"):
